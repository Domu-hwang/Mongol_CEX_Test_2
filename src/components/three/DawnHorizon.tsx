...
const fragmentShader = `
  ...
  float particle(vec2 uv, vec2 center, float size, float time, vec2 aspect) {
    ...
    // Bright glowing core - stronger
    float core = exp(-dist * dist / (size * size * 0.06));

    // Soft outer glow - larger
    float glow = exp(-dist * dist / (size * size * 0.5));

    // Combine - brighter
    float p = core * 1.5 + glow * 0.7;

    // Strong sparkling twinkle effect
    float twinkle1 = sin(time * 2.5 + center.x * 50.0 + center.y * 35.0) * 0.4;
    float twinkle2 = sin(time * 3.2 + center.x * 25.0 - center.y * 40.0) * 0.35;
    float twinkle3 = cos(time * 2.0 + center.x * 60.0) * 0.3;
    float twinkle4 = sin(time * 4.0 + center.y * 45.0) * 0.25;
    float twinkle = 0.5 + twinkle1 + twinkle2 + twinkle3 + twinkle4;
    twinkle = clamp(twinkle, 0.1, 1.8);
  ...
  void main() {
    ...
    // Create galaxy particle field with many stars
    for (int i = 0; i < 150; i++) {
      float fi = float(i);
      ...
      // Varying sizes - small star-like particles with some variation
      float sizeRand = h3;
      float size = 0.001 + sizeRand * sizeRand * 0.006;

      float p = particle(vUv, particlePos, size, uTime + fi, aspect);

      // Brightness - brighter overall and near mouse
      float brightness = 0.8 + h2 * 0.5 + mouseProximity * 1.0;
    ...
    // Add particles with good visibility
    color += particleColor * particleMask * (1.2 + mouseInfluence * 0.9);
    alpha = max(alpha, particleAlpha * particleMask * 0.85);
...
const DawnMesh: React.FC<DawnMeshProps> = ({ mousePosition, isHovering }) => {
  ...
  // Calculate scale to maintain 16:9 aspect ratio (cover behavior)
  ...
  if (currentAspect < targetAspect) {
    // Portrait mode (mobile) - extend width to maintain ratio
    scaleX = viewport.height * targetAspect;
  } else {
    // Landscape mode - extend height to maintain ratio
    scaleY = viewport.width / targetAspect;
  }
...
const DawnHorizon: React.FC<DawnHorizonProps> = ({ className = '' }) => {
  ...
  // Ensure canvas renders after container is mounted and sized
  useEffect(() => {
